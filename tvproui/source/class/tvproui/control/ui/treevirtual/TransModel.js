...
/* ************************************************************************
************************************************************************ */
qx.Class.define('tvproui.control.ui.treevirtual.TransModel',
{
  ...
  construct : function(columnNames)
  {
    ...
    /* 初始化数据 */
    this.base(arguments, columnNames);

    /* 初始化变更记录 */
    this._undoList = new qx.data.Array();
    this._redoList = [];
    this._transList = [];
  },
  events :
  ...
  members :
  {
    ...
    commitUpdate : function()
    {
      ...
      /* 插入并执行更新数据事务 */
      var result = this._pushTrans(this._getNewUpdateCommand(this._oldMap, this._newMap));
      if (!result) {
    ...
    _clearUndoList : function()
    {
      ...
      undoList.removeAll();
      undoList.push(new tvproui.control.ui.command.GroupCommand([], "加载完毕"));
