 ...
 * 更新命令
 */
qx.Class.define("tvproui.control.ui.treevirtual.command.UpdateCommand",
{
  ...
  members :
  {
    ...
    _readyServerData : function(map, map2)
    {
      ...
      for (ID in map)
      {
        ...
        for (col in colMap)
        {
          ...
          // 客户端专有列不外发
          if (model.IsClientOnlyCol(col)) {
            continue;
          }
          var value = colMap[col];
          var columnName = model.getColumnVarName(col);
    ...
    _executeClient : function(map)
    {
      ...
      for (ID in map)
      {
        ...
        for (col in colMap)
        {
          ...
          col = parseInt(col);
          var value = colMap[col];
          model.setColumnData(ID, col, value);
          model.updateRowData(ID, node, row, col, value);
        }
      }
      model.setData();
      return true;
    ...
    {
      this.base(arguments);
      return this._executeClient(this._oldMap);
    }
  },
  destruct : function()
  {
