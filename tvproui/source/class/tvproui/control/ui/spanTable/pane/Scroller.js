 ...
 */
qx.Class.define("tvproui.control.ui.spanTable.pane.Scroller",
{
  ...
  */
  members :
  {
    ...
    _createChildControlImpl : function(id, hash)
    {
      ...
      switch (id)
      {
        case "header":control = (this.getTable().getNewTablePaneHeader())(this);
        break;
        case "pane":control = (this.getTable().getNewTablePane())(this);
        break;
        case "focus-indicator":control = new tvproui.control.ui.spanTable.pane.FocusIndicator(this);
        ...
        this.__paneClipper.add(control);
        break;
        case "scrollbar-x":control = this._createScrollBar("horizontal").set( {
          alignY : "bottom"
        });
    ...
    onTableModelDataChanged : function(firstRow, lastRow, firstColumn, lastColumn)
    {
      ...
      if (rowCount != this.__lastRowCount)
      {
        ...
        if (this.getFocusedRow() >= rowCount) {
          if (rowCount == 0) {
            this.setFocusedCell(null, null, null);
          } else {
            this.setFocusedCell(this.getFocusedColumn(), rowCount - 1, 1);
          }
        }
    ...
    __handleMoveColumn : function(pageX)
    {
      ...
      if (this.__header.isShowingColumnMoveFeedback() || pageX > this.__lastMoveMousePageX + clickTolerance || pageX < this.__lastMoveMousePageX - clickTolerance)
      {
    ...
    _onMousedownPane : function(e)
    {
      ...
      {
        var tableModel = table.getTableModel();
        var node = tableModel.getDisplayDesc(col, row);
        ...
        if (selectBeforeFocus) {
          table.getSelectionManager().handleMouseDown(row, level, e);
        }
    ...
    _onMouseupPane : function(e)
    {
      ...
      if (row != -1 && row != null && col != null)
      {
        ...
        var level = node.level;
        row -= node.offset;
        table.getSelectionManager().handleMouseUp(row, level, e);
      }
    },
    ...
    _onClickPane : function(e)
    {
      ...
      if (row != null && col != null)
      {
        ...
        var level = node.level;
        row -= node.offset;
        table.getSelectionManager().handleClick(row, level, e);
        if (this.__focusIndicator.isHidden() || (this.__lastMouseDownCell && !this.__firedClickEvent && !this.isEditing() && row == this.__lastMouseDownCell.row && col == this.__lastMouseDownCell.col))
        {
      ...
      } else
      {
        ...
        // Datouxia Added For TVPRO, resetSelection When select blank area
        if (row == null)
        {
          table.setFocusedCell(null, null);
          table.getSelectionModel().resetSelection();
    ...
    _onContextMenu : function(e)
    {
      ...
      if (!this.getShowCellFocusIndicator() || row === null || (this.__lastMouseDownCell && row == this.__lastMouseDownCell.row && col == this.__lastMouseDownCell.col))
      {
    ...
    startEditing : function()
    {
      ...
      if (!this.isEditing() && (col != null) && tableModel.isColumnEditable(col))
      {
        ...
        } else
        {
          ...
          this.__focusIndicator.addListener("mousedown", this._onStopMouseDown, this);
    ...
    flushEditor : function() {
      if (this.isEditing())
      {
        ...
        var oldValue = tableModel.getValue(col, row);
        tableModel.setValue(col, row, value);
        this.__table.focus();
        var tableModel = this.getTable().getTableModel();
        var node = tableModel.getNodeByRowColumn(col, row);
        var columnID = tableModel.getColumnId(col);
    ...
    cancelEditing : function() {
      if (this.isEditing() && !this.__cellEditor.pendingDispose) {
        ...
        } else
        {
          ...
          this.__focusIndicator.removeListener("mousedown", this._onStopMouseDown, this);
          this.__cellEditor.destroy();
          this.__cellEditor = null;
          this.__cellEditorFactory = null;
        }
     ...
     *
     * @return {qx.ui.table.pane.Clipper} The pane clipper for this scroller.
     */
    ...
    _updateFocusIndicator : function()
    {
      ...
      var pane = this.__tablePane;
      var firstRow = pane.getFirstVisibleRow();
      var rowCount = pane.getVisibleRowCount();
      var rowSpace = rowCount - (this.__focusedRow - firstRow);
      var span = this.__focusedSpan > rowSpace ? rowSpace : this.__focusedSpan;
      this.__focusIndicator.moveToCell(this.__focusedCol, this.__focusedRow, span);
    }
  },
  ...
  */
  destruct : function()
  {
    ...
    this._disposeObjects("__horScrollBar", "__verScrollBar", "__headerClipper", "__paneClipper", "__focusIndicator", "__header", "__tablePane", "__top", "__timer", "__clipperContainer");
  }
});
