...
/* ************************************************************************
************************************************************************ */
qx.Class.define('tvproui.material.MaterialModel',
{
  ...
  construct : function()
  {
    ...
    /* 初始化列信息 */
    this.setColumns(["ID", "名称", "类型", "时长", "失效", "编号", "上传人", "标签", "资源ID"], ["ID", "name", "type", "duration", "endTime", "artId", "alias", "tag", "resourceID"]);
  ...
  members :
  {
     ...
     */
    loadData : function(resourceTreeID)
    {
      if(0 > resourceTreeID){
        dialog.Dialog.error("错误的资源目录!");
        return;
      }

      this.setResoureID(resourceTreeID);
      var rows = tvproui.AjaxPort.call("Material/loadMaterial",
        {
